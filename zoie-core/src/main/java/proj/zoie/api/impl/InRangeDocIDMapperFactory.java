...
public class InRangeDocIDMapperFactory implements DocIDMapperFactory {
      ...
      for (int i = 0; i < subreaders.length; ++i) {
        ...
        for (int k = 0; k < subuidarray.length; ++k) { // k is the local DOC ID for the subreader
          ...
          if (subid != ZoieIndexReader.DELETED_UID) {
            int local_uid = (int) (subid - _partStart); // this is local (local to partition and not
                                                        // to subreader)
            // relative UID index in the partition
            if ((local_uid < 0) || (local_uid >= docidArray.length)) {
              log.error("Local UID outof range for localUID: " + local_uid + " _start: "
                  + _partStart + " _count: " + _count + " " + (long) subid);
            }
            docidArray[local_uid] = k + start;// global DOCID
            subReaderIndex[local_uid] = i; // set the sub-reader index
          }
        }
        subreader.setDocIDMapper(new DocIDMapperSub(i, subreader, _partStart, docidArray, start));
      }
  ...
  public static final class DocIDMapperSub implements DocIDMapper<DocIDArray> {
     ...
     */
    public DocIDMapperSub(int id, ZoieSegmentReader<?> subreader, long _partStart, int[] uidArray,
        int start) {
    ...
    public final int getDocID(long uid) {
      ...
      if (mapped != DocIDMapper.NOT_FOUND) {
        if (mapped >= max) return DocIDMapper.NOT_FOUND;
        return mapped - start;
      }
  ...
  public static final class DocIDMapperGlobal implements DocIDMapper<DocIDArray> {
     ...
     */
    public DocIDMapperGlobal(long partiStart, int[] uidArray, ZoieSegmentReader<?>[] subreaders,
        int[] starts, int[] subreaderindex) {
  ...
  public static final class DocIDMapperSmall implements DocIDMapper<DocIDArray> { // do the
                                                                                  // samething as
