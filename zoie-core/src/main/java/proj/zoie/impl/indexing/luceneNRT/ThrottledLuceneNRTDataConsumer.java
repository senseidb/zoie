...
public class ThrottledLuceneNRTDataConsumer<D> implements LifeCycleCotrolledDataConsumer<D>,
  ...
  */
  ...
  public ThrottledLuceneNRTDataConsumer(Directory dir, Analyzer analyzer,
      ZoieIndexableInterpreter<D> interpreter, long throttleFactor, MergePolicy mergePolicy) {
    _writer = null;
    _analyzer = analyzer;
    ...
    if (events.size() > 0) {
      for (DataEvent<D> event : events) {
        ...
        if (!_appendOnly) {
          try {
            _writer
                .deleteDocuments(new Term(DOCUMENT_ID_FIELD, String.valueOf(indexable.getUID())));
          } catch (IOException e) {
      ...
      while (!_stop) {
        ...
        if (ThrottledLuceneNRTDataConsumer.this._writer != null) {
          try {
            ...
            ThrottledLuceneNRTDataConsumer.this._currentReader = IndexReader.open(
              ThrottledLuceneNRTDataConsumer.this._writer, true);
